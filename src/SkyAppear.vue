...
export default {
    ...
    props: {
        ...
        /* tag */ // ? Maybe allow the use of an optional wrapper?
        options: {
            type: Object,
    ...
    computed: {
        ...
        computedClassList() {
            return (this.active && !this.computedOptions.classless) ? this.classList : [];
        },
        computedOptions() {
            // The set options overwriting the defaults
            return Object.assign(Object.assign({}, defaultOptions), this.options);
        },
        rootMarginObject() {
            // Transforming the root margin string into an array of more usable values
            let rootMarginArray = this.computedOptions.rootMargin.split(' ').filter(segment => segment);

            rootMarginArray = rootMarginArray.map(item => {
                if (item.match(/(^-?[0-9]*\.?[0-9]+)px/g) && item.match(/(^-?[0-9]*\.?[0-9]+)px/g).length === 1) {
                    return {
                        value: item.match(/(^-?[0-9]*\.?[0-9]+)/g)[0],
                    };
                }
                if (item.match(/(^-?[0-9]*\.?[0-9]+)%/g) && item.match(/(^-?[0-9]*\.?[0-9]+)%/g).length === 1) {
                    return {
                        value: item.match(/(^-?[0-9]*\.?[0-9]+)/g)[0],
                        isPercentage: true,
                    };
        ...
        observerResetters() {
            ...
            const { root, rootMargin, thresholds } = this.computedOptions;
            return { active: this.computedActive, root, rootMargin, thresholds };
        },
    },
    ...
    methods: {
        observerCallback(obs, forceAccept= { value: false }) {
            ...
            if (!delay || forceAccept.value || (performance.now() - this.lastTime >= delay)) {
                // Try to generate an observation, if it doesn't currently exist
                if (!obs) {
                    const newObs = this.compileObserverData();
                    if (newObs && (delay || this.computedAppearOnce || !newObs[0].intersectionRect.width == !newObs[0].isIntersecting)) {
                        obs = newObs;
                    }
                }
                ...
                if (obs && obs.length) {
                    ...
                    if (isIntersecting) {
                        ...
                        switch (this.direction) {
                            // Vertical ratio
                            case 'block': {
                                this.intersectionRatio = intersectionRect.height / boundingClientRect.height;
                            } break;
                            // Horizontal ratio
                            case 'inline': {
                                this.intersectionRatio = intersectionRect.width / boundingClientRect.width;
                            } break;
                            // Both axi ratio
                            case 'both': {
                                this.intersectionRatio = (intersectionRect.height / boundingClientRect.height) * (intersectionRect.width / boundingClientRect.width);
                            } break;
                        }
                    } else {
                        switch (this.direction) {
                            ...
                            case 'inline': {
                                if (boundingClientRect.right < rootBounds.left) {
                                    this.state = 'before';
                                    this.isBeforeViewport = true;
                                } else if (boundingClientRect.left > rootBounds.right) {
                                    this.state = 'after';
                                    this.isAfterViewport = true;
                                } else {
                                    this.state = 'outside';
                    ...
                    } else if (this.state === 'before' || this.state === 'after') {
                        classList.push('sky-appear--outside-viewport');
                        classList.push(`sky-appear--outside-viewport--${this.state}`);
                    } else {
                    ...
                    if (this.computedOptions.appearOnce && this.state === 'inside') {
                        this.activeOverwrite = false;
                    }

                    // Compile data for emission
                    const observer = { // ! UPDATE IN DOCUMENTATION
                        time,
                        rootBounds,
                        boundingClientRect,
                        intersectionRect,
                    ...
                    if (this.state !== statePrevious) {
                        // Enter
                        if (this.state === 'inside') {
                            this.$emit('enter', Object.assign( { type: 'enter' }, observer));
                        }

                        // Update
                        this.$emit('update', Object.assign( { type: 'update' }, observer)); // ? Not sure if this should exist here?

                        // Leave
                        if (this.state !== 'inside') {
                            this.$emit('leave', Object.assign( { type: 'leave' }, observer));
                        }
                    } else {
                        // Update
                        this.$emit('update', Object.assign( { type: 'update' }, observer));
                    }
        ...
        resetObserver() {
            ...
            // Setup new observer
            if (this.computedActive && this.$el && this.$el.nodeType !== 8) { // ".nodeType !== 8" is for avoiding comments
                this.observer = this.observer || new IntersectionObserver(this.observerCallback, this.observerOptions);
        ...
        updateActiveTriggerEvents(events = []) {
            ...
            const eventsToRemove = oldEvents.filter(event => {
                return newEvents.findIndex((element) => {
                    return
            ...
            eventsToRemove.forEach((event) => {
                event.target.removeEventListener(event.name, this.eventCallback);
            });

            // Add events
            const eventsToAdd = newEvents.filter(event => {
                return oldEvents.findIndex((element) => {
                    return
                        element.name === event.name
                        && element.target === event.target
                        && element.options === event.options
            ...
            eventsToAdd.forEach((event) => {
                event.target.addEventListener(event.name, this.eventCallback, event.options);
            });
        },
        eventCallback() {
        ...
        compileObserverData() {
            if (this.$el && this.$el.nodeType !== 8) { // ".nodeType !== 8" is for avoiding comments
                // Get bounds of observed element,  root and intersection area
                // Observed element bounds
                const boundingClientRect = this.$el.getBoundingClientRect();

                // Root element bounds
                const root = this.computedOptions.root || document.documentElement;
                const rootBounds = JSON.parse(JSON.stringify(root.getBoundingClientRect()));

                if (!this.computedOptions.root) {
                    Object.assign(rootBounds, { left: 0, x: 0, top: 0, y: 0, right: rootBounds.width, bottom: rootBounds.height });
                }

                // Add root margins
                const margin = {
                    top: this.rootMarginObject.top.isPercentage
                        ? this.rootMarginObject.top.value * rootBounds.height / 100
                        : this.rootMarginObject.top.value * 1,
                    right: this.rootMarginObject.right.isPercentage
                        ? this.rootMarginObject.right.value * rootBounds.width / 100
                        : this.rootMarginObject.right.value * 1,
                    bottom: this.rootMarginObject.bottom.isPercentage
                        ? this.rootMarginObject.bottom.value * rootBounds.height / 100
                        : this.rootMarginObject.bottom.value * 1,
                    left: this.rootMarginObject.left.isPercentage
                        ? this.rootMarginObject.left.value * rootBounds.width / 100
                ...
                intersectionRect.right = Math.max(intersectionRect.left, Math.min(boundingClientRect.right, rootBounds.right));
                intersectionRect.bottom = Math.max(intersectionRect.top, Math.min(boundingClientRect.bottom, rootBounds.bottom));
